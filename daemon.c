...
int randEntropy()
{
  ...
  syslog(LOG_INFO, "Entropy:: time = %ld, freeram = %ld", sys_info.uptime, sys_info.freeram);

  num += ((int)sys_info.uptime * (int)sys_info.freeram) ^ randFunction(myRand(0, 999));
    ...
    if( !strcmp(argv[1], "start") )
    {
      ...
      else
      {
        ...
        while(1)
        {
          ...
          {
            syslog( LOG_INFO, "SizeFile %i < %i ", countSizeFile(nameDataFile), 5 * M_BYTES );
